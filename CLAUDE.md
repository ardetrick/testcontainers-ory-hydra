...
A single package (`com.ardetrick.testcontainers`) with a deliberately small public surface, enforced by ArchUnit:

- **`OryHydraContainer`** — Extends `GenericContainer`. Runs database migration and the Hydra server in a single container using a compound command (`migrate && serve`). Defaults to SQLite so no external database is required. A `Builder` inner class configures the Docker image, Hydra environment variables (`URLS_LOGIN`, `URLS_CONSENT`, `URLS_SELF_ISSUER`, `URLS_LOGOUT`, `SECRETS_SYSTEM`, `DSN`, plus arbitrary env vars), wait strategy, and declarative client fixtures (`client(Map)` — Hydra client JSON passed through verbatim, upserted right after startup). `URLS_LOGIN`/`URLS_CONSENT` default to non-resolvable sentinel hosts (never contacted; the flow driver intercepts challenges by query parameter). Exposes the flow-helper factories `clientCredentialsFlow()` / `authorizationCodeFlow()`, plus `introspect(String)` and `openIdConfiguration()`.

- **Flow and result types** (`AuthorizationCodeFlow`, `ClientCredentialsFlow`, `FlowResult`, `HydraFlowException`, `IntrospectionResponse`, `OpenIdConfiguration`) — framework-agnostic pure JDK code bound only to base URIs, so they work against any Hydra instance. `execute()` returns a `FlowResult`: `TokenResponse` (RFC 6749 §5.1) or `OAuthError` (§5.2) — protocol errors are values, transport/flow failures throw `HydraFlowException`. The authorization-code driver follows redirects manually, answers Hydra's login/consent challenges via the admin API, and rewrites each Hydra-bound redirect to the mapped host/port.

- **Everything else is package-private** (`Json`, `JsonWriter`, `Http`, `AdminClient`, `TokenEndpointClient`, `OAuth2Clients`) — the hand-rolled JSON code exists only to keep the library dependency-free and must not become public API.

Two ArchUnit rules in `ArchitectureTest` enforce this: only `OryHydraContainer` may depend on Testcontainers/Docker and nothing may depend back on the container (the dependency is strictly one-way), and only the whitelisted API types may be public. New public types require adding to that whitelist deliberately.
